...
type
   ...
   public
      ...
      function Deleta(const Id: Integer): Boolean;
   end;
...
function TModelEntidadePessoa.Deleta(const Id: Integer): Boolean;
   function DeletaEndereco(const Id: Integer): Boolean;
   var
      AModelEntidade: IModelEntidade;
      ASQL: String;
      AQuery: IModelQuery;
...
begin
   Open('id_pessoa = ' + Id.ToString);
   with fQuery do
      begin
         Result := not Query.IsEmpty;
         if Result then
            begin
               Result := DeletaEndereco(Id);
               if Result then
                  Query.Delete;
            end;
      end;
